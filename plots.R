...
selected_sd = 25
df_mod <- subset(df, init_sd == selected_sd & init_SNR > 1, select = c(PSNR, init_SNR, iter))
...
ggplot(df_mod, aes(x=iter, y=PSNR, colour=as.factor(init_SNR))) +
  ...
  guides(color=guide_legend(title='Initial SNR (dB)')) +
...
# geom_raster(aes(fill=PSNR), interpolate=TRUE)
